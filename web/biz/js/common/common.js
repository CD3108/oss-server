...
define(["jcl","mobile","clientTool"],function(Wade,Mobile,ClientTool) {
	var Common = new function(){
		...
		/*通过server-interface.xml配置访问接口*/
		this.callIf = function(action,param,callback,isEscape,error){
			param = param ? param : new Wade.DataMap();
            error = error ? error : function(x_info, x_code) {
				Mobile.loadingStop();
				if(x_code){
			...
			Common.get(function(data) {
				...
				callInterface("InterfaceBean.commonInterface", param, callback, isEscape, error);
			}, [Constant.SESSION_ID,Constant.STAFF_ID]);
			
			function callInterface(_action,_param,_callback,_isEscape,_error){
				Mobile.dataRequest(_action,_param,function(resultData){
					if (typeof (resultData) == "string") {
						resultData = new Wade.DataMap(resultData);
					}
		...
		this.openPageIf = function(pageAction,ifAction, param, error) {
			param = param ? param : new Wade.DataMap();
			error = error ? error : function(x_info, x_code) {
				Mobile.loadingStop();
				if(x_code){
			...
			Common.get(function(data) {
				...
				callInterface("InterfaceBean.commonInterface", param, error);
			}, [Constant.SESSION_ID,Constant.STAFF_ID]);
			
			function callInterface(_action,_param,_error){
				Mobile.dataRequest(_action,_param,function(resultData){
					if (typeof (resultData) == "string") {
						resultData = new Wade.DataMap(resultData);
					}
					...
					if(x_resultcode<0){
						...
						Mobile.openTemplate(pageAction,resultData);
					}
				}, _error);
			}
		};
		...
		this.logoutAccount = function(){
			if(confirm("确定要退出吗?")){
				...
				/*Mobile.openTemplate("OssLogin");*/
				Mobile.openPage("OssLogin");
			}
		};
