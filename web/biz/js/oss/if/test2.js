require(["domReady!","mobile", "util"], function(doc,Mobile) {
	...
	var dbName = "test", tableName = "student";
	//数据信息
	var Person = {
	...
	$("#create").tap(function() {
		...
		Mobile.execSQL(dbName, createSql, null, null, null, function(result){
			if("YES" != result)
				alert("新建表失败[" + result + "]！");
			//开放功能
			funcControl("insert", "e_button-blue");
			funcControl("drop", "e_button-red");
		});
	});
	//查询数据
	var queryAll = function() {
		...
		WadeMobile.select(dbName,tableName,null,null,null,null,null,function(obj){
			alert("sss:"+obj)
			qryResultDeal(obj, columns);
			});
	};
	//查询第一条数据
	...
	var qryResultDeal = function(obj, qryCols){
		...
		for(var i = 0, length = result.length; i < length; i++){
			...
			childObjStr += "<td><button class='e_button-red' stu_no='" + stuNo + "'>➖</button></td></tr>";
		}
		//将子元素节点添加到表格中
		$("#tbody").append($(childObjStr));
		
		//绑定删除按钮元素动作
		$("#tbody").find(".e_button-red").each(function(){
			$(this).tap(function() {
	...
	var funcControl = function(obj, classStr){
		var operObj = $("#" + obj);
		if(operObj.hasClass("e_button-blue"))
			operObj.removeClass("e_button-blue");
		else if(operObj.hasClass("e_button-red"))
			operObj.removeClass("e_button-red");
		else if(operObj.hasClass("e_button-cancel"))
			operObj.removeClass("e_button-cancel");
		
		operObj.addClass(classStr);
		
		operObj.attr("disabled", ("e_button-cancel" == classStr));
	};
	//初始化动作
	funcControl("insert", "e_button-cancel");
	funcControl("drop", "e_button-cancel");
